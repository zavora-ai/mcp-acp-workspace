...
impl AcpStore {
    ...
    pub fn expose_agent(&self, agent_name: &str, acp_server_name: &str, transport: AcpTransport, bind_addr: Option<String>) -> ExposedAgent {
        // In production, this spawns the adk-acp server binary:
        //   acp-server-example (or custom binary built with adk-acp server feature)
        // The binary path can be configured via ACP_SERVER_BINARY env var.
